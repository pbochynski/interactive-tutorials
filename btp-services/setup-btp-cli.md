...
SM_PLAN=$(btp list services/plan | jq -r '( [ .[] | select(.name=="service-operator-access" and .service_offering_name=="service-manager") | .id ] | .[0] )')
SOA_INSTANCE=$(btp list service/instance | jq --arg plan_id_var "$SM_PLAN" -r '( [ .[] | select(.service_plan_id == $plan_id_var) | .id ] | .[0] )')
...
echo "Service operator access instance ID: ${SOA_INSTANCE}"
```{{exec}}

Now get the credentials for the service instance. If you don't have any binding yet, create one:
```
BINDING_CREDENTIALS=$(btp list service/binding --fields-filter "service_instance_id eq '${SOA_INSTANCE}'" | jq -r '(.[0]?.credentials)')
...
The credentials are saved in `creds.json` file. Check if the file is created and contains the credentials:
```
cat creds.json | jq -r '.'
```{{exec}}
